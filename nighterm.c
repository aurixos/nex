                ...
                void *(*custom_malloc)(size_t))
{
    ...
    config.terminal[config.current_terminal].buffer = nighterm_malloc(framebuffer_height * framebuffer_width * (framebuffer_bpp >> 3));
#else
    (void)custom_malloc;
#endif
 ...
 *              Blue color (0-255)
 */
void nighterm_priv___putpixel(uint64_t x, uint64_t y, uint8_t r, uint8_t g, uint8_t b)
{
 ...
 */
void nighterm_render_char(int row, int col, char c)
{
    int rounding = ((config.terminal[config.current_terminal].font_header.width % 8) != 0) ^ (config.terminal[config.current_terminal].font_header.width == 9);
    uint8_t *glyph = config.terminal[config.current_terminal].font_data + c * config.terminal[config.current_terminal].font_header.charSize;

    for (size_t y = 0; y < config.terminal[config.current_terminal].font_header.height; y++)
    {
        for (size_t x = 0; x < config.terminal[config.current_terminal].font_header.width; x++)
        {
            if ((glyph[y * ((config.terminal[config.current_terminal].font_header.width / 8) + rounding) + x / 8] >> (7 - x % 8)) & 1)
            {
                ...
                nighterm_priv___putpixel(col * config.terminal[config.current_terminal].font_header.width + x, row * config.terminal[config.current_terminal].font_header.height + y, r, g, b);
            }
            else
            {
...
void nighterm_write(char c)
{
    ...
    case '\t':
        ...
        break;
    case '\b':
        nighterm_render_char(config.terminal[config.current_terminal].cur_y, config.terminal[config.current_terminal].cur_x, ' ');
    ...
    default:
        ...
        if (config.terminal[config.current_terminal].cur_x >= config.terminal_cols)
        {
            config.terminal[config.current_terminal].cur_x = 0;
            config.terminal[config.current_terminal].cur_y++;
        }
        nighterm_render_char(config.terminal[config.current_terminal].cur_y, config.terminal[config.current_terminal].cur_x, ch);
